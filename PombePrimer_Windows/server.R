...
primer3path <- file.path('ResourcesAndDownloads', 'Primer3release-2.3.6')
rootdir <- getwd()

#Primer3 caller funtion
func.primer3 <- function(a) {
  on.exit(setwd(rootdir)) #change back to rootdir when the primer3 function finishes
  setwd(file.path(getwd(), primer3path)) #On Linux you can config the directory, but on Windows you need to set the working dir to primer3 or otherwise it won´t work
  #a = p3.input
  try(system2('primer3_core.exe', a, stdout=T))
}
...
##Update Gibsonmodules(), primers() and GibsonAssemblies() to use dropdown list-supplied vector bacbones instead of custom Gibson modules
##add function to manually update workspace with fresh genome coordinates and data
##add plasmid maps to a 'construct' tab on the mainPanel?
##add a tab to mainPanel with the PCR-Gibson protocol